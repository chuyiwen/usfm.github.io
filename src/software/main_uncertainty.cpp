...
#include <gflags/gflags.h>
    ...
    DEFINE_string(alg, "NULLSPACE_BOUNDING",
...
int main(int argc, char* argv[]) {
    ...
    Scene scene = Scene();
	scene._options._algorithm = cov::EAlgorithm_stringToEnum(FLAGS_alg);
    if ( !io->read( FLAGS_in, scene ) ) 
        exit(1);
        
    // compose Jacobian if not exist
    if( scene._jacobian.values.size() == 0 )
        JacobianComposer::scene2Jacobian( FLAGS_cam, scene );
    ...
    // COMPUTE COVARIANCES
	if (scene._options._algorithm == cov::eAlgorithmNullspaceBounding)
		computeNullspaceBounding(scene, statistic);
	else
