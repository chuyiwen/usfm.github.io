...
/*
Split matrix to 3 submatrices defined by boundary (usefull for symmetric matrices)
A = [_sA(0..border_x-1, 0..border_y-1) H1]
B = [_sA(0..border_x-1, border_y..end) H2]
C = _sA(border_x..end, border_y..end)
*/
void ScaledSparseMatrix::splitTo3BlocksWithH(Scene &s, int border_x, int border_y, Eigen::MatrixXd &H, ScaledSparseMatrix *A, ScaledSparseMatrix *B, ScaledSparseMatrix *C) {
	int numA = 0, numB = 0, numC = 0;
	int N = nrows(), M = ncols();
	...
	numA += 21 * s._options._numCams;	// add related values in H
	numB += 9 * s._options._numPoints;  // add related values in H

	// create vectors for matrices 
	A->set_sA(std::make_shared<CRS>(border_x + 7, border_y + 7, numA));
	auto csrA = A->get_sA();
	B->set_sA(std::make_shared<CRS>(border_x, M - border_y, numB));
	auto csrB = B->get_sA();
	C->set_sA(std::make_shared<CRS>(N - border_x, M - border_y, numC));
